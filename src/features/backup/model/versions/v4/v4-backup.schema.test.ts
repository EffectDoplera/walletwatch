...
import { V4Backup, v4BackupConsistentSchema } from "./v4-backup.schema";
...
describe("v4 backup validation", () => {
  ...
  describe("currencies", () => {
    ...
    it("without precision", () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const backup: any = generateBackup();
      delete backup.currencies.currencies[currencyId1].precision;
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(false);
    });
    ...
    it("precision is not integer", () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const backup: any = generateBackup();
      backup.currencies.currencies[currencyId1].precision = 1.1;
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(false);
    });

    it("precision is gte 0", () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const backup: any = generateBackup();
      backup.currencies.currencies[currencyId1].precision = -1;
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(false);
    });

    it("precision is not number", () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const backup: any = generateBackup();
      backup.currencies.currencies[currencyId1].precision = "invalid";
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(false);
    });
  ...
  describe("settings", () => {
    ...
    it("without appearance", () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const backup: any = generateBackup();
      delete backup.settings.appearance;
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(false);
    });

    it("without appearance.textSize", () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const backup: any = generateBackup();
      delete backup.settings.appearance.textSize;
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(false);
    });

    it("appearance.textSize is invalid enum", () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const backup: any = generateBackup();
      backup.settings.appearance.textSize = "invalid";
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(false);
    });

    it("appearance.textSize is large", () => {
      const backup = generateBackup();
      backup.settings.appearance.textSize = "large";
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(true);
    });

    it("appearance.textSize is small", () => {
      const backup = generateBackup();
      backup.settings.appearance.textSize = "small";
      const { success } = v4BackupConsistentSchema.safeParse(backup);
      expect(success).toBe(true);
    });
  });
